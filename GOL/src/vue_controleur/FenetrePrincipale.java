...
public class FenetrePrincipale extends JFrame implements Observer {
    ...
    public FenetrePrincipale(Environnement env) {
        ...
        environnement = env;
        environnement.addObserver(this);
        construireInterface();
    }
    ...
    public void construireInterface() {
        ...
        for (int i = 0; i < environnement.getSizeX(); i++) {
            for (int j = 0; j < environnement.getSizeY(); j++) {
                ...
                grille[i][j].setBackground(Color.WHITE);  // Par défaut
        ...
        sliderVitesse.setSnapToTicks(false);
        ...
        JLabel labelGlider = new JLabel("               Glider"); //espaces pour afficher au centre
        ...
        // Ajouter les boutons et les labels
        int spacing = 30;
    ...
    public int[] obtenirCoordonneesDepuisSouris(int xSouris, int ySouris) {
        ...
        int x = ySouris / cellHeight; //  ligne
        int y = xSouris / cellWidth;  // col
    ...
    public void afficherSurbrillanceCellule(int x, int y, boolean etat) {
        if (etat) {
            grille[x][y].setBackground(new Color(173, 216, 230)); //  bleu gris
        }
    }
    ...
    public void chargerEtat(ActionEvent e) {
        ...
        int retour = fileChooser.showOpenDialog(this);  // this --> FenetrePrincipale
        if (retour == JFileChooser.APPROVE_OPTION) {
            File file = fileChooser.getSelectedFile();
            XMLHandler.loadState(environnement, file.getAbsolutePath());
            rafraichir();  // Maj affichage après le chargement
        }
    }
